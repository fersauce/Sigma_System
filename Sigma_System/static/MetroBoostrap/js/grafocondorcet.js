...
function grafo_condorcet (id, votaciones) {
	...
	var dibuja_nodo = function (centro, etiqueta) {
		...
		canvas.stroke();
		canvas.font = parseInt(radio)+'px Times New Roman 40% monoespace';
		canvas.fillText(etiqueta, centro.x-(radio/1), centro.y+(radio/3));
	}
	...
	var dibuja_flecha = function (origen, destino, etiqueta) {
		...
		canvas.stroke();
		canvas.font = parseInt(radionodo/1.5)+'px Times New Roman ';
		canvas.fillText(" ", (destino_x+origen_x)/2, (destino_y+origen_y)/2);
	}
